...
export function checkCodexPorcelain(codexRoot, deliverableFiles) {
  ...
  for (const line of gitPorcelain(codexRoot)) {
    ...
    if (!allowed.has(path)) {
      violations.push({
        ...
        line: trimmed,
        path,
        reason: trimmed.startsWith('??') ? 'untracked_outside_deliverable' : 'tracked_outside_deliverable',
      });
    }
  }
  return violations;
...
export function checkRepoPorcelain(repoPath, repoName) {
  const violations = [];
  for (const line of gitPorcelain(repoPath)) {
    ...
    violations.push({
      ...
      line: trimmed,
      path,
      reason: trimmed.startsWith('??') ? 'untracked' : 'tracked_modified',
    });
  }
  return violations;
}
...
export function auditGoalScope(codexRoot, opsRoot) {
  ...
  const codexPorcelain = checkCodexPorcelain(codexRoot, deliverable);
  const codexCommits = checkGoalCommits(codexRoot, deliverable);
  const akalynthPorcelain = existsSync(akalynthRoot) ? checkRepoPorcelain(akalynthRoot, 'akalynth') : [];
  const agentSkillsPorcelain = existsSync(agentSkillsRoot)
    ? checkRepoPorcelain(agentSkillsRoot, 'akalynth-agent-skills')
    : [];

  const violations = [...codexPorcelain, ...codexCommits, ...akalynthPorcelain, ...agentSkillsPorcelain];
  return {
